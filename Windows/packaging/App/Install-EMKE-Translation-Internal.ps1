...
function Resolve-ExactBundleInventory {
    ...
    if (-not [string]::Equals(
        ...
        throw "The running installer is not the exact bundled helper."
    }
...
function Assert-InternalPackageAbsent {
    ...
    if ($null -ne $package) {
        throw "The exact Internal package is already installed."
    }
}
...
function Invoke-InstallRollback {
    ...
    try {
        ...
        $errors.Add("exact install-record rollback failed")
    }
    return [pscustomobject]@{
        Complete = $errors.Count -eq 0
        RecoveryRequired = $errors.Count -ne 0
        Summary = $errors -join "; "
    }
}
...
function Invoke-InstallInternalMsix {
    ...
    $trustedThumbprint = $ExpectedCertificateThumbprint.ToUpperInvariant()
    ...
    try {
        ...
        $rollback = Invoke-InstallRollback `
            ...
            -ExpectedCertificateSha256 $inventory.CertificateSha256 `
            -ExpectedCertificateThumbprint $trustedThumbprint
        $recovery = if ($rollback.Complete) {
            "Exact rollback completed; retry the original trusted bundle."
        } else {
            (
                "Recovery required: rerun the bundled uninstall helper with " +
                "-RemoveCertificate, -ConfirmRemoveCertificate, and the " +
                "same fixed -ExpectedCertificateThumbprint."
            )
        }

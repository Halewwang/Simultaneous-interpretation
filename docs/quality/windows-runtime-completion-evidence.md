...
| Alias, malformed event, server close | `TranslationEventCodecTests.DecodeRejectsLegacyPreV024EventNames`, malformed and closed-registry rows; `TranslationRuntimeIntegrationTests.LoopbackUsesFrozenTranslationEventsAndRejectsUnregisteredAliases`, `RejectedProductionHandshakeStopsRuntimeWithStableSafeRoute` (`LegacyAlias`), `MalformedProductionBinaryHandshakeStopsRuntimeAndDrainsAudioOwnership`, `ProductionServerCloseFencesOutboundAndReleasesRuntimeOwnership`; `TranslationSessionTests.ConnectedTransportCloseBecomesRetryableNetworkFailure` | The full Runtime handshake rejects legacy aliases as `protocol` / `translationEvent.unknownType` / `retry` and binary frames as `binaryTranslationEvent`; both end in safe `Stopped` routes before audio ownership. A real production server close fences the outbound virtual microphone, retains inbound translation, then proves socket/poll/queue/lease cleanup after stop. |

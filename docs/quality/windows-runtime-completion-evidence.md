...
| Alias, malformed event, server close | `TranslationEventCodecTests.DecodeRejectsLegacyPreV024EventNames`, malformed and closed-registry rows; `TranslationRuntimeIntegrationTests.LoopbackUsesFrozenTranslationEventsAndRejectsUnregisteredAliases`, `RejectedProductionHandshakeStopsRuntimeWithStableSafeRoute` (`LegacyAlias`), `BinaryProductionHandshakeStopsRuntimeAndDrainsAudioOwnership`, `ProductionRawMalformedTextFencesOutboundAndDrainsOwnership`, `ProductionCloseFrameFencesOutboundWithUnexpectedSocketClose`; `TranslationSessionTests.ConnectedTransportCloseBecomesRetryableNetworkFailure` | The full Runtime handshake rejects legacy aliases as `protocol` / `translationEvent.unknownType` / `retry`; binary framing is separately rejected as `binaryTranslationEvent`. Raw malformed Text reaches the Runtime as `protocol` / `translationEvent.invalidJson` / `retry`. A real WebSocket close output frame reaches it as `network` / `translationSession.unexpectedSocketClose` / `retry`; it fences outbound, retains inbound translation, and then proves socket/poll/queue/lease cleanup after stop. |
...
quiescence after shutdown. `AssertReleasedAsync` waits only on deterministic
per-language server connection-closed signals and the audio poll-quiesced
signal; `WaitAsync` remains a hang guard rather than scheduling wall-clock
polling.

`FailureSafetyTests.OneHundredDeterministicSeedsKeepVirtualMicrophoneZero`
also freezes category/code/recovery/empty-parameter contracts individually for
outbound disconnect, server error, send and receive failures, outbound queue
full, translated underrun, and device change; CloseTimeout separately freezes
its quarantine contract.
...
| Task 5 review re-review: close frame, raw malformed Text, deterministic ownership barriers | Runtime RED `30742387758` / job `91481983094` (tests referenced the intentionally absent seams) | `840addc`, `0c0e344`; Runtime `30742489577` / job `91482265599` passed all managed suites and MSIX `30742489586` / job `91482275579` succeeded; PR #22 is closed unmerged after capture. |

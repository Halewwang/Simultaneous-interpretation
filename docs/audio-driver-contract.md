...
or re-signing packaged bytes. Its regression test snapshots the complete bundle
contents, modes, xattrs, and code-signing metadata before and after validation.
It may still rebuild and locally sign the separate `.build/driver/verify-bundle`
smoke executable.

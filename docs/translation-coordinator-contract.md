...
| 语言决定期限 | 译音开始返回后 250 ms；仍未分类时，当前话语曾由 VAD 启动且已有译音则选译音，否则选原音；话语级语音来源在 VAD 结束后的尾部窗口内不回退为非语音 |
...
- 自动出站直通只有在本地音频引擎的 `setRouting` 返回后才发布为 `bypassed` 并形成 `canSpeak`；仅音频引擎启动不能提前形成可发言状态。
...
启动期间主状态可以继续显示全局启动进度，但入站和出站行始终投影各自的真实通道状态；一条通道已经 active 时，另一条通道仍在握手不能把前者重新显示为 connecting。协调器事件队列绑定运行 lifecycle；直接重启会在线性发布新 `connecting` 前丢弃上一 lifecycle 的排队事件，已取消的 waiter 不能消费新 lifecycle 的首个事件。
